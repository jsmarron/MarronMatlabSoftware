...
static void solve_l2r_l1l2_svc(
	...
	double Cp, double Cn, int solver_type, int **iteration)
{
	int l = prob->l;
	int w_size = prob->n;
	int i, s, iter = 0;
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn, int **iter)
{
	...
	switch(param->solver_type)
	{
		...
		case L2R_L2LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L2LOSS_SVC_DUAL,iter);
            break;
		case L2R_L1LOSS_SVC_DUAL:
			solve_l2r_l1l2_svc(prob, w, eps, Cp, Cn, L2R_L1LOSS_SVC_DUAL, iter);
            break;
		case L1R_L2LOSS_SVC:
		{
			problem prob_col;
...
model* train(const problem *prob, const parameter *param)
{
	...
	if(check_regression_model(model_))
	{
		...
		model_->nr_class = 2;
		model_->label = NULL;
		train_one(prob, param, model_->w, 0, 0, &iteration);
	}
	else
	{
		int nr_class;
		...
		else
		{
			if(nr_class == 2)
			{
				...
				train_one(&sub_prob, param, model_->w, weighted_C[0], weighted_C[1], &iteration);
			...
			else
			{
				...
				for(i=0;i<nr_class;i++)
				{
					...
					train_one(&sub_prob, param, w, weighted_C[i], param->C, &iteration);
