function [stat,pval,zscore] = DiProPermSMnew(mdata1,mdata2,paramstruct) ;
...
%                         6  DWDLarge - 2016 Fast version of DWD
%
...
for it = 1:nt ;
  ...
  elseif iidir == 6 ;    %  Support Vector Machine direction vector
    ...
    dirstr = 'DWDFast' ;
    y = [ones(size(mdata1,2),1); -ones(size(mdata2,2),1)] ; 
    [C,ddist] = penaltyParameter([mdata1 mdata2],y,1); 
    vdir = genDWDweighted([mdata1 mdata2],y,C,1) ;
  ...
  for isim = 1:nsim ;
    ...
    for it = 1:nt ;
      ...
      elseif iidir == 6 ;    %  Support Vector Machine direction vector
        ...
        dirstr = 'DWDFast' ;
        y = [ones(sum(comboss1flag),1); -ones(sum(~comboss1flag),1)] ; 
        [C,ddist] = penaltyParameter(mdata,y,1); 
        vdirc = genDWDweighted([mdata(:,comboss1flag) mdata(:,~comboss1flag)],y,C,1) ;
            %  DWD Fast direction vector, pointing from 2nd group towards first
