...
%   Steve Marron's & Xi Yang's matlab function
...
%   This improves DiProPermSM by:
%       - Allowing Balanced Permutations 
%              (more powerful in strong signal settings)
%       - Allowing FastDWD 
...
%                         6  DWDLarge direction vector 
%                                (a new DWD algorithm for solving large 
%                                 scale problems, and generalized DWD)                 
%
...
%                                       or by nsim = 0 
%                     2  -  Similar to 1, except show smallest and biggest statistic 
...
%                     Note:  When this is 'o', and ishowperm = 1 (or 2),
%                            For projections of permutations 1 & 2 (or small-big),
...
%    Copyright (c) J. S. Marron & Xi Yang 2005-2021
...
for it = 1:nt ;
  ...
  elseif iidir == 6 ;    %  Support Vector Machine direction vector
    ...
    S = rng ;
        ...
        %  since DWDLarge seems to reset it
  ...
  for isim = 1:nsim ;
    ...
    for it = 1:nt ;
      ...
      elseif iidir == 4 ;    %  Fisher Linear Discrimination direction vector
        ...
        mresid1 = mdata(:,comboss1flag) - ...
                        vec2matSM(vmean1,size(mdata(:,comboss1flag),2)) ;
        mresid2 = mdata(:,~comboss1flag) - ...
                        vec2matSM(vmean2,size(mdata(:,~comboss1flag),2)) ;
        mresid = [mresid1 mresid2] ;
        mcov = cov(mresid') ;
            %  Get covariance matrix, transpose, since want 
            %               "coordinates as variables"
...
pauseSM
        ...
        elseif isim == 2 ;
          ...
          if statperm1 <= statperm2 ;
            ...
            Perm12Cell{2,it} = statperm1 ;
            Perm12Cell{3,it} = vmarkerstr1 ;
            Perm12Cell{4,it} = mcolorsim1 ;
            Perm12Cell{5,it} = vdirperm2 ;
            Perm12Cell{6,it} = statperm2 ;
            Perm12Cell{7,it} = vmarkerstr2 ;
            Perm12Cell{8,it} = mcolorsim2 ;
          else ;
            statmin = statperm2 ;
            statmax = statperm1 ;
            Perm12Cell{1,it} = vdirperm2 ;
            Perm12Cell{2,it} = statperm2 ;
            Perm12Cell{3,it} = vmarkerstr2 ;
            Perm12Cell{4,it} = mcolorsim2 ;
            Perm12Cell{5,it} = vdirperm1 ;
            Perm12Cell{6,it} = statperm1 ;
            Perm12Cell{7,it} = vmarkerstr1 ;
            Perm12Cell{8,it} = mcolorsim1 ;
          end ;
          ...
          elseif statmax < statperm ;    %  then replace max by this
            ...
            mcolorsim = mcolor(indperm,:) ;
            Perm12Cell{5,it} = vdirperm ;
            Perm12Cell{6,it} = statperm ;
            Perm12Cell{7,it} = vmarkerstr ;
            Perm12Cell{8,it} = mcolorsim ;
          end ;
...
%    zscore = [zscore; (stat(it) - simmean) / simsd] ;
    ...
    if sum(mstat(:,it) == mstat(1,it)) == length(mstat(:,it));
             %  if entries of mstat are all the same, return zscore of 0
      zscore = [zscore; 0] ;
