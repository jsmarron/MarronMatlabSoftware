...
if  nargout == 0  | ...
  ...
  if  ~(idatovlay == 0)   &  imptyp >= 0  ;    %  then add data to plot
    ...
    hold on ;
      ...
      if isstr(dolcolor) ;    %  then can plot colors individually
        if ibigdot == 1 ;   %  plot deliberately large dots
          ...
          plot(datatrunc,hts,[dolcolor '.']) ;
        end ;
      else ;    %  need to plot points individually
        for idol = 1:length(datatrunc) ;
          if ibigdot == 1 ;   %  plot deliberately large dots
            plot(datatrunc(idol),hts(idol),'o','MarkerSize',1,'LineWidth',2,'Color',dolcolor(idol,:)) ;
          else ;    %  use matlab default dots
            plot(datatrunc(idol),hts(idol),'.','Color',dolcolor(idol,:)) ;
          end ;
        end ;
      end ;
