disp('Running MATLAB script file SigClustCovEstSMtest.m') ;
%
%    FOR DEVELOPMENT AND TESTING OF MATLAB FUNCTION SigClustCovEstSM
%     Does soft thresholded COVariance ESTimate 


itest = 37 ;     %  1,  2,3,4,5,6,7,8    very simple tests
...
if itest == 1 ;    %  Simple toy data, just for construction purposes
  ...
  [veigvest, tau] = SigClustCovEstSM(vsampeigv,sig2b) 
...
elseif   itest == 21  | ...
  ...
  plot(vi,vsampeigv,'ko','MarkerSize',8,'LineWidth',2) ;
    title(['Test Graphic for SigClustCovEstSM,   itest = ' num2str(itest)],'FontSize',15) ;
    axisSM(vi,vsampeigv) ;
    vax = axis ;
    hold on ;
...
elseif   itest == 31  | ...
  ...
  for isim = 1:nsim ;
    ...
    %
    [veigvest, tau] = SigClustCovEstSM(veigval,sig2b) ;
    vtau = [vtau; tau] ;
