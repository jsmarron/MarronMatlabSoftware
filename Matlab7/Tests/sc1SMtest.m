disp('Running MATLAB script file sc1SMtest.m') ;
%
%    Test program for sc1SM.m
...
itestpar = 14 ;  %  test various parameter settings (when ipart = 0)
                %  1 kde, entered xgrid
...
if ipart == 0 ;   %  then do simple tests
  ...
  elseif itestpar == 10 ;
    ...
    paramstruct = struct('vxgp',[0,1,40], ...
                         'vhgp',[.1,.1,1]) ;
    vecout = sc1SM(data,paramstruct) ;
    vecout
  elseif itestpar == 11 ;
    data = [rand(20,1)/4; 0.25+rand(180,1)/4; 0.5+rand(20,1)/4; 0.75+rand(180,1)/4] ;
    subplot(2,2,1) ;
...
%function [mapout,xgrid] = sc1SM(data,vxgp,vhgp,eptflag,alpha,simflag,llflag) 
