...
%    Copyright (c) J. S. Marron 2003-2023, Chris Cabanski 2009-2010
...
if nargin > 2   %  then paramstruct is an argument
  ...
  if isfield(paramstruct,'imeantype')    %  then change to input value
    imeantype = paramstruct.imeantype ; 
  end
  ...
  if isfield(paramstruct,'legcellstr')    %  then change to input value
    legcellstr = paramstruct.legcellstr ; 
  end
...
%  adjdata(:,flagn) = rawdata(:,flagn) - vec2matSM(meanprojn * dirvec,length(vprojn)) ;
  ...
  if viplot(1)    %  then make "before" 2-d scatterplot matrix
    ...
    if ~isempty(savestr)
      paramstruct.savestr = [savestr 'PCA2dBefore'] ;
    end

    if ~isempty(legcellstr)
      paramstruct.legendcellstr = legcellstr ;
    end
  ...
  if viplot(2)    %  then make DWD projection plot
    ...
    if ~isempty(savestr)
      paramstruct.savestr = [savestr 'DWDproj'] ;
    end
  ...
  if viplot(3)    %  then repeat DWD, for checking, and make projection plot
    ...
    if ~isempty(savestr)
      paramstruct.savestr = [savestr 'DWDrepeatProj'] ;
    end
...
%    fignum = fignum + 1 ;
    ...
    if ~isempty(savestr)
      paramstruct.savestr = [savestr 'PCA2dAfter'] ;
    end
