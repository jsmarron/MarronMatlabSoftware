...
%                         7  DWDLarge direction vector (Param as for idir = 1)
%
...
%                     2x3 color matrix, top row for Class 1, 
%                                           bottom row for Class 2
...
%                             will create single output file
%
...
%    Copyright (c) J. S. Marron & Xi Yang 2005-2023
...
for it = 1:nt 
  ...
  elseif iidir == 7    %  DWD Large direction vector, parametrized to give DWD2
    ...
    vdir = genDWDweighted(DWDLarge_X,DWDLarge_y,Ca,1,options);
    vdir = vdir./norm(vdir);
        %  DWD new algorithm direction vector, pointing from 2nd group towards first
    rng(S) ;
        %  restore state of random number generator
...
%    PDC = [PDC; (stat(it) - simmean) / simsd] ;
    ...
    if  ishowperm == 1  || ...
      ...
      subplot(2,2,4)
      vdirperm2 = Perm12Cell{4,it} ;
      statperm2 = Perm12Cell{5,it} ;
      vmarkerstr2 = vmarkerstr ;
      mcolorsim = Perm12Cell{6,it} ;
      paramstructPP1 = struct('icolor',mcolorsim, ...
                              'isubpopkde',1, ...
