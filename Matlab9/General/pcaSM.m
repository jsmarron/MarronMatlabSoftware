...
%                                   i.e. of "scores".
%                                   Note "low rank approx of data" from
%                                     vmean + meigvec * mpc
%                                   Common "PCi" mode of variation comes 
%                                   from meigvec(:,i) * mpc(i,:)
%                     Entry 6:  a3proj  -  d x n x npc  3d array of
%                                   projections of data onto individual
%                                   eigenvectors (modes of variation).
...
%    Copyright (c) J. S. Marron 2001, 2004, 2023
...
if size(viout,1) == 1    %  then have row vector
  ...
  elseif length(viout) > maxviout    %  give warning message
    disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
    disp('!!!   Warning from pcaSM:           !!!') ;
    disp('!!!   viout longer than expected    !!!') ;
    disp('!!!   will truncate, and proceed    !!!') ;
    disp('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!') ;
    viout = viout(1:maxviout) ;
  end
...
if iprestd ~= 1    %  then need to column object mean center

  vmean = mean(mdat,2) ;
          %  column object mean (i.e. across rows)
...
if iscreenwrite == 1
  disp('    pcaSM starting Singular Value Decomposition') ;
end

if npc < d    %  do reduced SVD

  [meigvec,S,V] = svds(mdat,npc) ;
...
if  viout(5) == 1  || ...
  ...
  elseif iorient == 3    %  then re-orient direction +- of eigenvectors
                           %  to make sum of entries positive
    ...
    for ipc = 1:npc    %  loop through eigenvectors
      ...
      if sum(meigvec(:,ipc)) < 0    %  then flip this eigenvector and the projections
        meigvec(:,ipc) = -meigvec(:,ipc) ;
        mpc(ipc,:) = -mpc(ipc,:) ;
      end
...
if  viout(8) == 1  || ...
  ...
  ssmresid = sum(sum(mdat.^2)) ;
      %  Recall mdata has been object mean centered
...
if viout(10) == 1
  ...
  vpropSSpr = vpropSSmr(1) ;
      %  first proportion is same as above

  for iev = 2:npc

    frac = (vpcSS(iev) / vpcSS(iev - 1)) ;
    if frac < (10^(-12))    %  any difference with 0
...
if viout(1) == 1
  outstruct.veigval = veigval ;
end

if viout(2) == 1
  outstruct.meigvec = meigvec ;
end

if viout(3) == 1
  outstruct.vmean = vmean ;
end

if viout(4) == 1
  outstruct.mmeanresid = mdat ;
      %  Recall mdata has been object mean centered
end
...
if viout(6) == 1
  outstruct.a3proj = a3proj ;
end

if viout(7) == 1
  outstruct.sstot = sstot ;
end

if viout(8) == 1
  outstruct.ssmresid = ssmresid ;
end

if viout(9) == 1
  outstruct.vpropSSmr = vpropSSmr ;
end

if viout(10) == 1
  outstruct.vpropSSpr = vpropSSpr ;
end
