...
%    Copyright (c) J. S. Marron, Jan Hannig, Cheolwoo Park 1997-2023
...
%}
  integrand = @(x,A,B) (exp(-x).*(1+erf((A-x/(2*A))/sqrt(2))) ...
                               .*(1+erf((B-x/(2*B))/sqrt(2)))/4) ;
...
%}
    numind=numind+integral(@(x)integrand(x, ...
                               sqrt(3*log(ngrid*nh))*delta/(2*vh(iint)), ...
                               sqrt(3*log(ngrid*nh))*(vh(2)/vh(1)-1)/2), ...
                           0,1000);
  end
