...
%    Note:  seems key conclusion is that when DWDlarge is modified to look like DWD2, it ends up witb MD!  See tests 3,4,5,6  (but not quite 7)

%    Should next look for examples where DWD2 is clearly better than MD.  Perhaps HDLSS with large variation in direction different fom mean?
...
itest = 14 ;     %  1 - simple test
                ...
                %  23 - Like 17, but n1 = 200, n2 = 50, d = 1000
...
elseif itest == 3 ;    %  Xi Yang no signal example, compare with DWD2 
  ...
  disp(['Angle DWDLarge & DWDLarge (mod) dirns = ' ...
                     num2str(180 * acos(abs(vdir' * vdira)) / pi)]) ;
  ...
  disp(['Angle DWDLarge (mod) & MD dirns = ' ...
...
elseif  itest == 17  | ...
  ...
  testfactor = 100 ;
      %  factor of how much bigger test data should be
  ...
  disp(['Proportion of - cases misclassified by DWD2(L) = ' num2str(DWD2LerrNtest)]) ;
